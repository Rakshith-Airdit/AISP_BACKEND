  ...
  try {
    ...
    while (true) {
      const page = await db.run(
        `ZP_AISP_POVIM_OCR_HD?$top=${limit}&$skip=${skip}`
      );
      ...
      for (let item of page) {
        ...
        const Dpamt = new Decimal128(item.Dpamt);
        const Dppct = new Decimal128(item.Dppct);
        bulkOps.push({
          updateOne: {
            filter: { Ebeln: ebeln },
            update: { $set: { ...item, Ebeln: ebeln, Dpamt, Dppct } },
            upsert: true,
          },
        });
      }
      ...
      if (bulkOps.length > 0) {
        try {
          ...
          console.warn(`⚠️ Batch insert warning (header):`, err.message);
        }
      }
...
async function fetchAndStorePOOCRItem() {
  ...
  try {
    ...
    while (true) {
      const page = await db.run(
        `ZP_AISP_POVIM_OCR_itm?$top=${limit}&$skip=${skip}`
      );
      ...
      for (let item of page) {
        ...
        const unitprice = new Decimal128(item.UnitPrice);
        const ktmng = new Decimal128(item.Ktmng);
        const menge = new Decimal128(item.Menge);
        const netpr = new Decimal128(item.Netpr);
        const netwr = new Decimal128(item.Netwr);
        const brtwr = new Decimal128(item.Brtwr);
        const kbetr = new Decimal128(item.Kbetr);
        const grnqty = new Decimal128(item.GRN_QTY);
        const totalamnt = new Decimal128(item.TOTAL_AMT);
      ...
      if (bulkOps.length > 0) {
        try {
          ...
          console.warn(`⚠️ Batch insert warning:`, err.message);
        }
      }
...
module.exports = async (srv) => {
    ...
    if (action === "CREATE") {
      ...
      try {
        ...
        const NPoBasedRequestInfo = {
          ...PoVimhead[0],
          INVOICE_AMOUNT: new Decimal128(PoVimhead[0].INVOICE_AMOUNT),
          DOWNPAYMENT_AMOUNT: new Decimal128(PoVimhead[0].DOWNPAYMENT_AMOUNT),
          REQUEST_NO,
          STATUS: 25, // uploaded by vendor
          LAST_UPDATED_ON: new Date(),
          CREATED_ON: new Date(),
          PROCESS_LEVEL: "FIC_PO_OCR",
        ...
        const VIM_PO_ITEM_WITH_OCR_INSERT = PoVimitem?.length
          ? insertData(
              ...
              PoVimitem.map((data) => ({
                ...
                INVOICED_PRICE: new Decimal128(data.INVOICED_PRICE),
                REQUEST_NO,
              }))
            )
      ...
      try {
            ...
            { REQUEST_NO: RNumber },
            {
              $set: {
                ...
                DOWNPAYMENT_AMOUNT: new Decimal128(
                  PoVimhead[0].DOWNPAYMENT_AMOUNT
                ),
                LAST_UPDATED_ON: new Date(),
        ...
        if (PoVimitem && Array.isArray(PoVimitem) && PoVimitem.length > 0) {
          ...
          const newItems = PoVimitem.map((item) => ({
            ...item,
            TOTAL_PRICE: new Decimal128(item.TOTAL_PRICE),
            INVOICED_PRICE: new Decimal128(item.INVOICED_PRICE),
            REQUEST_NO: RNumber,
          }));
  ...
  srv.on("triggerOCR", async (req) => {
    ...
    try {
      ...
      // Use correct FormData and append stream
      ...
      const ocrResponse = await axios.post(
        "https://batman-aisp.cfapps.ap10.hana.ondemand.com/invo/",
        form,
        {
          headers: {
            "X-Internal-API-Key": "P9iJd0^^hqpZu_o=sMwfU/}omBq'WXl",
            ...form.getHeaders(),
          },
        }
      );
      ...
      console.log("Data comming from AI services", ocrData);

      const invoiceDetails = ocrData?.Success?.Output?.invoiceDetails;
