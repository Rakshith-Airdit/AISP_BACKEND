...
// ────────────────────────────────────────────────
// Date converter to handle /Date(1654320000000)/ to ISO format
...
const SES_COLLECTION_HEAD = "SES_VIM_HEAD_DATA";

// Common handler to fetch filtered data for given STATUS
const handleReadTab = (statusCode) => async (req) => {
  const { database } = await getConnection();

  // Build the MongoDB query from the CAP request
  let { filter, select, sort, skip, limit } = buildMongoQuery(req);

  const finalFilter = { ...filter, STATUS: statusCode };

  // Count total entries for pagination
  const totalCount = await database
    .collection(SES_COLLECTION_HEAD)
  ...
  const data = await database
    ...
    .project({
      ...
      STATUS_DESC: 1,
      STATUS: 1,
      SAP_INVOICE_NUMBER: 1,
      CREATED_ON: 1,
      CURRENT_ASSIGNEE: 1,
    })
    .sort({ ...sort, REQUEST_NO: -1 })
    .skip(skip)
    .limit(limit)
    .toArray();
...
module.exports = async (srv) => {
  ...
  srv.on("READ", "SESVimHead", async (req) => {
    ...
    try {
      const data = await mongoRead("SES_VIM_HEAD_DATA", req, [
        { SESVimItem: "SES_VIM_ITEM_DATA" },
        { SESDetails: "SES_VIM_DETAILS_DATA" },
        { SESVimAttachments: "SES_VIM_ATTACHMENTS" },
      ]);
      return data;
    } catch (err) {
  ...
  srv.on("READ", "SESDetails", async (req) => {
    ...
    try {
      const data = await mongoRead(COLLECTION_DETAILS, req, []);
      if (!data) {
        return req.error({
          code: 404,
          message: `No data found!!`,
  ...
  srv.on("GET", "SES_VIM_APPROVAL_LOGS", async (req) => {
    const { client, database } = await getConnection();

    try {
      const data = await mongoRead("SES_VIM_APPROVAL_LOGS", req, []);
      if (!data) {
        return req.error({
          code: 404,
          message: `No data found!!`,
  ...
  // Status eq 3
  srv.on("READ", "REJECTED_TAB", async (req) => {
    const { client, database } = await getConnection();
    try {
      const data = await mongoRead("SES_VIM_HEAD_DATA", req, []);
      if (!data) {
  ...
  // Status eq 4
  srv.on("READ", "PENDING_TAB", async (req) => {
    const { client, database } = await getConnection();
    try {
      const data = await mongoRead("SES_VIM_HEAD_DATA", req, []);
      if (!data) {
  ...
  // Status eq 5
  srv.on("READ", "APPROVED_TAB", async (req) => {
    const { client, database } = await getConnection();
    try {
      const data = await mongoRead("SES_VIM_HEAD_DATA", req, []);
      if (!data) {
...
async function UpdateAttachments(
  ...
  console.log(
    `Deleted ${existingAttachments.length} old DB attachment(s) for ${REQUEST_NO}`
  );
