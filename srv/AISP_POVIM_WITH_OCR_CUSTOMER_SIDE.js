...
module.exports = async (srv) => {
      ...
      try {
        ...
        if (result && result.ApprovalLevel === maxApproverLevel) {
          const { supplierInvoiceRefNo, error } =
            await sendPostPayloadToSAPforCreateVimWithOCR(RNumber);
          if (error) {
          ...
          try {
            ...
            if (finalApprover) {
              ...
              return `Invoice created and Invoice ref no is  ${supplierInvoiceRefNo}.`;
            }
          } catch (err) {
  ...
  srv.on("GET", "VIM_PO_OCR_HEAD", async (req) => {
    ...
    try {
      ...
      console.error("Error reading PO OCR HEAD data:", err);
