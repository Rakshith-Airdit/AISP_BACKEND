...
module.exports = async (srv) => {
    ...
    if (action === "CREATE") {
      ...
      try {
        ...
        const NPoBasedRequestInfo = {
          ...NPoVimhead[0],
          TOTAL_AMOUNT: new Decimal128(TOTAL_AMOUNT),
          REQUEST_NO,
          // SOURCE_TYPE:'02-Portal',  //Need
  ...
  srv.on("READ", "NPoVimHeadList", async (req) => {
  ...
  srv.on("READ", "VENDOR_STATUS_F4HELP", async (req) => {
    const { database } = await getConnection();
    const collection = database.collection("VIM_NPO_HEAD_DATA");

    try {
      // Fetch distinct values from Mongo
      const distinctStatuses = await collection.distinct("VENDOR_STATUS");

      // Return in expected OData format
      return distinctStatuses.map((status) => ({ VENDOR_STATUS: status }));
    } catch (err) {
      console.error("Error reading SES Vim data:", err);
      req.error({ code: 500, message: err.message });
    }
